                              ...
                              const std::string &host)
{
    ...
    if (addr_handle)
    {
        ...
        while (addrinfo)
        {
            ...
            {
                inet_ntop(AF_INET, &((reinterpret_cast<struct sockaddr_in *>(addrinfo->ai_addr))->sin_addr), ip_str,
                          IP_STR_LEN);
                sin_ctx->ips.emplace_back(ip_str);
            } else if (addrinfo->ai_family == AF_INET6 && sin_ctx->ipv6)
            {
                inet_ntop(AF_INET6, &((reinterpret_cast<struct sockaddr_in6 *>(addrinfo->ai_addr))->sin6_addr), ip_str,
                          IP_STR_LEN);
                sin_ctx->ipsv6.emplace_back(ip_str);
            }
                                         ...
                                         const std::vector<std::string> &host_list)
{
    ...
    if (gather_ctx->ipv4)
    {
        ...
        WFGoTask *pwork_v4_task = WFTaskFactory::create_go_task("batch", HDFactory::start_dns_paralell, server_task,
                                                                true);
    ...
    if (gather_ctx->ipv6)
    {
        ...
        WFGoTask *pwork_v6_task = WFTaskFactory::create_go_task("batch", HDFactory::start_dns_paralell, server_task,
                                                                false);
    ...
    if (addr_handle)
    {
        ...
        while (addrinfo)
        {
            if (ipv4 && addrinfo->ai_family == AF_INET)
            {
                inet_ntop(AF_INET, &((reinterpret_cast<struct sockaddr_in *>(addrinfo->ai_addr))->sin_addr), ip_str,
                          IP_STR_LEN);
                dns_ctx->ips.emplace_back(ip_str);
